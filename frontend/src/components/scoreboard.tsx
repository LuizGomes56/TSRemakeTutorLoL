...
const Scores = ({ x, y }: { x: Ply[], y?: (i: number) => void }) => (
    x.map((p, i) => {
        ...
        let s = p.scores;
        let d = c.name + i;
        let r = y ? `has-[:checked]:odd:bg-[#221418] has-[:checked]:even:bg-[#26121C] cursor-pointer odd:bg-[#0E1C1C] even:bg-[#032625]` : "darkblue:odd:bg-[#111827] odd:bg-stone-900 darkblue:even:bg-[#1a253b] even:bg-[#242427]";
        return (
            <label
                htmlFor={d}
                key={d}
                className={`text-zinc-300 darkblue:text-slate-300 sm:justify-between even:border-b even:border-b-[#525252] even:border-t even:border-t-[#525252] grid grid-cols-2 gap-2 p-2 transition-all duration-200 ${r}`}
            >
                <input
                    type="checkbox"
                    id={d}
                ...
                <div className="flex items-center">
                    ...
                    <span className="min-w-0 mx-1.5 hidden sm:flex flex-col justify-between h-[26px]">
                        ...
                        <p className="truncate text-[10px] darkblue:text-slate-400 text-zinc-400 leading-none">{p.championName}</p>
                    </span>
                </div>
...
export default function Scoreboard({ game, setChecked, code }: { game: DataProps, setChecked: React.Dispatch<React.SetStateAction<boolean[]>>, code: string }) {
    ...
    return (
        <div className="shade">
            <span onClick={copyToClipboard} className="hover:bg-zinc-800 transition-all duration-200 cursor-pointer flex font-bold gap-2 items-center justify-center darkblue:text-slate-300 text-zinc-300 p-4 darkblue:bg-slate-900 bg-zinc-900">
                {!copied && <img className="h-5" src="/copy.svg" alt="" />}
                <p>{copied ? `Code copied to clipboard` : `Game code - ${code}`}</p>
            </span>
            <div className="grid grid-cols-2 bg-zinc-900 darkblue:bg-slate-900">
                {["Allies", "Show / Hide Enemies"].map((x, i) => (
                    <div key={x + i} className="h-12 flex items-center justify-center">
                        <span className="font-bold dropshadow darkblue:text-slate-300 text-zinc-300">{x}</span>
                    </div>
                ))}
                <div><Scores x={t} /></div>
