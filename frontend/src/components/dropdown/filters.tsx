...
export default function Filters({ onFilter }: { onFilter: (stat: string) => void }) {
    return (
        <div className="flex flex-col items-center rounded gap-1">
            {allStats.map((s, i) => (
                <label htmlFor={s + i} className="has-[:checked]:bg-neutral-700 transition-all duration-200 flex items-center justify-center h-8 w-8 hover:bg-slate-700 bg-neutral-800 cursor-pointer">
